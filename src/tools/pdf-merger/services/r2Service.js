...
export async function uploadToR2(file) {
  ...
  const res = await fetch('/r2-presign', {
    ...
    body: JSON.stringify({ filename: file.name, contentType: file.type || 'application/pdf', tool: 'pdf-merger' }),
  })
  ...
  const { presignedUrl, key, pdfMergerBackendUrl } = await res.json()
  ...
  return { presignedUrl, key, pdfMergerBackendUrl }
}
